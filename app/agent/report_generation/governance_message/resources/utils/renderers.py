...
def render_TD_요약_NPS진단(data: Any) -> str:
    ...
    nps_context = {
        ...
        "벤치마크사_NPS_차이": f"{bench_nps_diff:.1f}",
        "벤치마크사_NPS_플래그": bench_nps_flag,
        "시장평균_NPS_차이": f"{market_nps_diff:.1f}",
        "시장평균_NPS_플래그": market_nps_flag,
        "비추천비중": f"{kb_detractor:.1f}",
        "벤치마크사_비추천비중_차이": f"{detractor_diff:.1f}",
...
def render_TD_NPS진단_NPS영향요인(data: Any) -> str:
    ...
    factor_context = {
        ...
        "최상위은행_CX_차이": f"{top_bank_cx_diff:.1f}",
        "최상위은행_CX_플래그": top_cx_flag,
        "벤치마크사명": benchmark_name,
        "벤치마크사_CX_차이": f"{bench_cx_diff:.1f}",
...
def render_TD_고객경험_고객경험_NPS진단(data: Any) -> str:
    ...
    if preprocessing_top_name != "KB국민은행":
        ...
        # 높음/낮음
        top_det_diff = f"{preprocess_top_det_diff:.1f}%p"
    ...
    context = {
        ...
        "직상위기관명": direct_name,
        "직상위기관_NPS_차이": f"{direct_nps_diff:.1f}",
        ...
        "직상위기관_비추천비중_차이": f"{direct_det_diff:.1f}",
...
def render_TD_고객경험_고객경험_영향요인(data: Any) -> str:
    ...
    factor_context = {
        ...
        "최상위은행_SQ_차이": f"{top_bank_diff:.1f}",
        "최상위은행_SQ_플래그": top_bank_flag,
        "벤치마크사명": benchmark_name,
        "has_benchmark": has_benchmark,
        "벤치마크사_SQ_차이": f"{bench_diff:.1f}" if has_benchmark else None,
...
def render_TD_고객경험_고객경험_고객감정(data: Any) -> str:
    ...
    context: Dict[str, Any] = {
        ...
        # NSS 영역
        "KB_NSS": f"{kb_nss:.1f}",
        "NSS점수_차이": f"{nss_diff:.1f}",
        "NSS_플래그": nss_flag,
        "부정비중_차이": f"{neg_diff:.1f}",
        "부정비중_플래그": neg_flag,
        # CCS 영역
        "KB_CCS": f"{kb_ccs:.1f}",
        "CCS점수_차이": f"{ccs_diff:.1f}",
        "CCS_플래그": ccs_flag,
        "개선비중_차이": f"{imp_diff:.1f}",
        "개선비중_플래그": imp_flag,
    }
...
def render_BU_채널NPS_NPS분석(data: Any) -> str:
    ...
    nps_total = data_by_type.get("bu_cha_nps_total", [])
    factor_tot = data_by_type.get("bu_cha_factor_total", [])
    ...
    if preprocessing_전월_대비_차이 > 0:
        전월_대비_차이 = f"전월대비 ({abs(preprocessing_전월_대비_차이):.1f})%p"
        상승_하락 = "상승"
    ...
    factor_rows = [r for r in factor_tot if r.get("채널") == channel_name]
...
def render_BU_고객경험_고객경험_품질요소(data: Any) -> str:
    ...
    factor_total = data_by_type.get("bu_cx_factor_total", [])

    # 고객경험단계
    cx_stage = factor_total[0].get("고객경험단계", "")

    # NPS 계산 (추천비율 기준으로 정렬)
    sorted_by_rec = sorted(factor_total, key=lambda x: float(x.get("추천비율", 0)), reverse=True)
    max_row = sorted_by_rec[0] if sorted_by_rec else {}
    min_row = sorted_by_rec[-1] if sorted_by_rec else {}

    # 전월영향도 값(숫자형) 추출
    preprocessing_max_monthly_impact = safe_float(max_row.get("전월대비영향도", 0))
    preprocessing_min_monthly_impact = safe_float(min_row.get("전월대비영향도", 0))

    # 상승/하락 여부 계산
    if preprocessing_max_monthly_impact > 0:
        max_monthly_impact = f"전월대비 {preprocessing_max_monthly_impact}점"
        상승폭 = "상승"
    elif preprocessing_max_monthly_impact == 0:
        max_monthly_impact = f"전월과"
        상승폭 = "동일"
    else:   # preprocessing_max_monthly_impact < 0 인 경우
        max_monthly_impact = f"전월대비 {preprocessing_max_monthly_impact}점"
        상승폭 = "하락"
    
    # 하락폭 계산
    if preprocessing_min_monthly_impact < 0:
        min_monthly_impact = f"전월대비 {preprocessing_min_monthly_impact}점"
        하락폭 = "하락"
    elif preprocessing_min_monthly_impact == 0:
    ...
    context: Dict[str, Any] = {
        ...
        "CX": cx_stage,
        # 누적 가장 높은 NPS 품질요소
        "누적_가장높은_추천_품질요소명": max_row.get("서비스품질요소", ""),
        "누적_가장높은_추천_품질요소점수": f"{safe_float(max_row.get('추천비율', 0)):.1f}",
        # 누적 가장 낮은 NPS 품질요소
        "누적_가장낮은_추천_품질요소명": min_row.get("서비스품질요소", ""),
        "누적_가장낮은_추천_품질요소점수": f"{safe_float(min_row.get('추천비율', 0)):.1f}",
        # 전월대비 상승·하락 폭
...
def render_BU_문제원인(data: Any) -> str:
    ...
    neg_record = max(factor_total, key=lambda r: float(r.get("영향도", 0)))
    neg_cx = neg_record.get("고객경험단계", "")
    neg_service = neg_record.get("서비스품질요소", "")

    # 영향도 가장 하위인 고객경험단계 / 서비스품질요소
    pos_record = min(factor_total, key=lambda r: float(r.get("영향도", 0)))
