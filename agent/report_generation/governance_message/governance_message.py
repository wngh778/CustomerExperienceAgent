...
class GovernanceMessage:
    ...
    async def generate_governance(
        ...
        try:
            ...
            base_date = content.get("date", datetime.now(GovernanceMessage.KST).strftime("%Y%m%d"))
            ...
            recent_data_date = await self.mcp_executor.execute_tool(
                "mysql_query", {"query": raw_sql.format(base_date=base_date)}
            )
            ...
            if recent_data_date:
                ...
                override = {
                    ...
                    "base_date": first_item["기준년월일"],
                }
            else:
                logger.error(f"[generate_governance] {base_date} 날짜 기준 가까운 과거 날짜가 없습니다.")
                raise Exception
            ...
            async for leaf_path, rendered_msg, code_dict, query_data in self._iterate_query_results(
                ...
                if code_dict.get("설문조사방식구분") == "01":
                    if override["semester"] == "상반기":
                        base_ym = str(override["survey_year"]) + "00"
                    else:
                        base_ym = str(override["survey_year"]) + "99"
                    base_dt = base_ym + "000000"
                else:
                    base_dt = override["base_date"]
                    base_ym = base_dt[:-2]
