...
SELECT 
    ...
    A.중립고객수,
    A.비추천고객수,
    ROUND(A.전체대비응답비중점수, 1) AS 전체대비응답비중,
    ROUND(A.전체대비추천비중점수, 1) AS 전체대비추천비중,
    ROUND(A.전체대비중립비중점수, 1) AS 전체대비중립비중,
    ROUND(A.전체대비비추천비중점수, 1) AS 전체대비비추천비중,
    ROUND(A.NPS영향도점수, 1) AS NPS영향도,
    COALESCE(B.벤치마크사, '-') AS 벤치마크은행구분
FROM inst1.TSCCVMGC7 A
