WITH TMP AS (
    SELECT
        ...
        , 채널명 AS 채널
        , 고객경험단계명 AS 고객경험단계
        , ROUND(SUM(긍정건수) * 100.0 / SUM(전체건수), 1) - ROUND(SUM(부정건수) * 100.0 / SUM(전체건수), 1) AS CXNSS
        , ROUND(SUM(긍정건수) * 100.0 / SUM(전체건수), 1) AS CX긍정비율
        , ROUND(SUM(부정건수) * 100.0 / SUM(전체건수), 1) AS CX부정비율
        , ROUND(SUM(불만건수) * 100.0 / SUM(전체건수), 1) AS CXCCI
        , ROUND(SUM(칭찬건수) * 100.0 / SUM(전체건수), 1) AS CX칭찬비율
        , ROUND(SUM(불만건수) * 100.0 / SUM(전체건수), 1) AS CX불만비율
...
SELECT
    ...
    , t1.고객경험단계
    , t1.CXNSS
    , ROUND(t1.CXNSS - t2.CXNSS, 1) AS 전주대비CXNSS변동
    , t1.CX긍정비율
    , t1.CX부정비율
    , t1.CXCCI
    , ROUND(t1.CXCCI - t2.CXCCI, 1) AS 전주대비CXCCI변동
    , t1.CX칭찬비율
    , t1.CX불만비율
FROM (
    SELECT *
